...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        //Important cors should come first than the csrf else the permit all was not working for the login url throwing invalid username
        http.cors().and().csrf().disable().exceptionHandling().authenticationEntryPoint(entryPoint).and()
                ...
                .antMatchers("/api/backlog/**").hasRole("admin")
                .antMatchers("/api/project/**").hasAnyRole("teller","user")
